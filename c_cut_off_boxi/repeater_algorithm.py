...
class RepeaterChainSimulation():
    ...
    def iterative_convolution(self, func, shift=0, first_func=None, p_swap=None):
        """
    ...
    def entanglement_swap(self,
            pmf1, w_func1, pmf2, w_func2, p_swap, cutoff, t_coh, cut_type
    ):
        """
        Calculate the waiting time and average Werner parameter with time-out for entanglement swap.
    ...
    def compute_unit(self,
        ...
        """
        Calculate the waiting time distribution and
        ...
        if "cutoff" in parameters.keys():
            ...
            cutoff = parameters.get("mt_cut", np.iinfo(np.int64).max)
        elif cut_type == "fidelity":
            cutoff = parameters.get("w_cut", 1.0e-16)  # shouldn't be zero
            if cutoff == 0.:
                cutoff = 1.0e-16
        elif cut_type == "run_time":
            cutoff = parameters.get("rt_cut", np.iinfo(np.int64).max)
        else:
            cutoff = np.iinfo(np.int64).max
    ...
    def nested_protocol(self, parameters, all_level=False):
        """
        Compute the waiting time and the Werner parameter of a symmetric repeater protocol.

        Parameters
        ----------
        parameters: dict
            A dictionary contains the parameters of the repeater and the simulation.
        all_level: bool
        ...
        if "cutoff_dict" in parameters.keys():
            ...
            mt_cut = cutoff_dict.get("memory_time", np.iinfo(np.int64).max)
            w_cut = cutoff_dict.get("fidelity", 1.e-8)
            rt_cut = cutoff_dict.get("run_time", np.iinfo(np.int64).max)
        else:
            mt_cut = parameters.get("mt_cut", np.iinfo(np.int64).max)
            w_cut = parameters.get("w_cut", 1.e-8)
            rt_cut = parameters.get("rt_cut", np.iinfo(np.int64).max)
